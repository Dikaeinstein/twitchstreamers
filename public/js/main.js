...
function buildStreamView (stream) {
    let template = `<div class="streamer row online">
<div class="icon column column-10"><img src=${stream.logo} /></div>
<div class="name column column-30"><a href=${stream.url} target="_blank">${stream.name}</a></div>
<div class="streaming column column-30">${stream.status}</div>
</div>`;
    return $(template);
}
...
function getResponse(res) {
    ...
    for (let val of res) {
        if (val.stream) {
            $(frag).append(buildStreamView(val.stream));
        }       
    }
    render(frag);
}
...
$(document).ready(function () {
  ...
  },
  {
    ...
    $("#online").click(function () {
        $(".streamer").hide().filter(".online").show();
    });
    $("#offline").click(function () {
        $(".streamer").hide().filter(".offline").show();
    });
